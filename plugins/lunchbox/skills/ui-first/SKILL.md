...
| Step | Sees | Source | Does | Risk |      ← Source names the section + row declaring each affordance in Sees AND Does, indicators included
Verdict: PASS | FAIL — most-likely-to-fail step: <step>
```

**Plain-token rule** (applies to both templates, same rationale as the backtick rule below): the principle rollup line and the `Grounding lookup:` line are emitted as plain text — `P3: FAIL`, `Grounding lookup: not run — <reason>` — with no emphasis markers anywhere in the label or the token. Emphasise the prose after them if you like. A rollup written `P3: **FAIL**` or `**P3: FAIL**` breaks the assertions that prove a planted violation was not waved through, and it breaks them *silently*, in the direction that reports a regression as a pass.
...
First-shot test rules (make it falsifiable, not theater): **in both modes, one walkthrough per primary actor's top task** — never a single global walkthrough when shells diverge. When one shared shell serves all roles, a single walkthrough of the most load-bearing actor's top task suffices. *Sees* and *Does* may name only affordances that literally exist in the produced structure — nav map, surface inventory, attention inbox, or disclosure stages — inventing one mid-walkthrough is a spec bug, not a pass. **The `Source` column is what enforces this**, and it is why the check is a column rather than an instruction: every affordance either cell names — nav item, tab, surface, action, count, badge — is listed there against the section and row that declares it. *Does* is in scope because the click a step names is an affordance too: an unsourced verb ("taps to start the job") is the same defect as an unsourced label, one column to the right.

An affordance with no source is a gap in the structure, not a wording problem: add it to the section that should have carried it and re-check, or rewrite the step to use what the structure actually produced. An empty, partial, or hand-waved `Source` cell is `FAIL` however well the steps read, and so is a near-match in a different word ("Start job" against a structure that says "checked in"; "Ready to invoice" against a tab declared as "Ready to send") — that mismatch is exactly what a first-shot user trips over. **An indicator carries its own source.** A badge or count is a separate affordance from the thing it counts, so citing the inbox or tab that supplies the *content* is only half the citation — the row declaring the *indicator* is the other half. "The Ready count also shows on the nav item" is a declaration; a bell that no row says is badged is not. Review mode sources against the ingested structure instead, naming the nav entry, route, or screen note. *Does* is the click a domain expert would choose knowing the domain but not the product. Hard-fail rule: any step whose correct next click requires an engine/internal term, or a location not derivable from the task, fails the test. The most-likely-to-fail step is named even on PASS. Review mode runs the same table against the actual ingested labels.
