...
| `id text [pk]` (TypeID) | `id text PRIMARY KEY` — **no DB default**; the app generates the TypeID, so every INSERT (seed and use-case) supplies an explicit prefixed string id. Do **not** add `gen_random_uuid()` or a `typeid_generate_*()` default (PGlite has neither). |
...
- **Never put a non-deterministic default (`now()`, `random()`) in a column you later
  ...
  instead. (TypeID PKs help here: they are explicit string literals, never a DB-generated
  default, so a PK is always a stable, assertable value.)
...
lists. Every PK is a TypeID, so **supply an explicit, type-prefixed string `id` for every
row** — there is no DB default or identity sequence to fall back on (this also removes the
whole `OVERRIDING SYSTEM VALUE` / sequence-reset class of foot-guns). Use stable, unique,
correctly-prefixed values; readable suffixes (e.g. `cust_alice`, `order_1001`) are
encouraged for test clarity — they are test fixtures, **not** format-valid TypeIDs (a real
suffix is 26 Crockford-base32 chars, e.g. `cust_01h5fskfsk4fpeqwnsyz5hj55t`), so don't add
a TypeID-shape `CHECK` that would reject them. A timestamp like `responded_at`/`created_at`
with `default: now()` is fine to include for realism — just never assert on it (see the
non-determinism rule in §1).
...
A write into a table with a TypeID PK must supply the explicit `id` (no DB default
generates it) — `INSERT INTO orders (id, customer_id, ...) VALUES ('order_2002', ...)`.
This also hands you a negative test for free: re-inserting an `id` that already exists is
a PK collision, so `INSERT … VALUES ('order_1001', …)` against a seeded id satisfies
`-- expect: error ~ unique`.
...
(`now()`, `random()`) appears in any asserted column or filter (TypeID PKs are explicit
literals, so the PK is never a source of nondeterminism). The
