...
The environment variable manifest. Generated by ddd-brief; maintained by hand as new
integrations are added. One row per variable — the canonical reference for `.env.example`
generation and for implementing agents that wire configuration loading.

```markdown
# Env manifest — <Project name>
<!-- ddd: env -->

| Variable | Type | Required-in | Default | Description |
|----------|------|-------------|---------|-------------|
| DATABASE_URL | string | all | none | PostgreSQL connection string |
| JWT_SECRET | string | staging, production | none | HMAC signing key for JWT tokens |
| STRIPE_SECRET_KEY | string | all | none | Stripe secret API key |
| USE_MOCK | bool | dev, test | false | Enable mock adapters (set via USE_MOCK=true) |

## Changelog
- 2026-06-25 (ddd-brief): created
```

Column semantics:
- `Variable` — exact env var name, `UPPER_SNAKE_CASE`.
- `Type` — `string`, `bool`, `int`, `url`, or `secret` (rotated credentials).
- `Required-in` — comma-separated environment names from `Environments:` in stack.md,
  or `all` (every environment), or `dev, test` (local only).
- `Default` — `none` when there is no safe default; a literal value otherwise.
- `Description` — one sentence: what it configures, not what it is named.

ddd-brief generates this file at greenfield time from the integrations and auth
