...
order, and their dependencies**. The Gherkin skill consumes this map to create
`features/<capability>/` folders and to order the suite — it does not re-derive
capabilities. This file does NOT define vocabulary (that is `glossary.md`) and does NOT
contain scenarios, responsibilities lists, or acceptance criteria (those are the
`.feature` files the Gherkin skill authors).

## Output

Copy `assets/capability-map-template.md` verbatim: one ordered table plus a one-line
walking-skeleton callout. See `assets/capability-map-example.md`.
...
4. **Capability name = folder name.** Use a short kebab-case name (`shift-swaps`); it
   becomes `features/shift-swaps/`.
5. **Outcome is one line** — the value the capability delivers, not a responsibilities list.
6. **Personas served are real.** Reference persona Roles/labels from `personas.md`.
7. **Name the walking skeleton.** Exactly one thinnest end-to-end slice, called out
   explicitly, with trivial behavior.
8. **No glossary, no scenarios, no folder-tree duplication.** The table's `Depends on`
   column IS the dependency record; do not restate it as prose.

## Downstream-purpose map

- **Capability + order** → `features/<capability>/` folders and suite build sequence.
...
- [ ] Names are kebab-case and folder-ready.
- [ ] Walking skeleton named exactly once.
