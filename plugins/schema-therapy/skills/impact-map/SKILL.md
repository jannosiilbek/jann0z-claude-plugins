...
name: impact-map
description: >-
  Step 0 of the schema-therapy modelling pipeline (the entry step) — produces
  specs/00-impact-map.md. Trigger to start the pipeline: "run the schema-therapy
  pipeline", "start the modelling pipeline", "impact map this product", "produce
  00-impact-map.md", or when handed a product intent to scope. Also trigger in
  Amend mode when an existing 00 must change: "add … to the scope", "amend the
  impact map", "update the scope document", "the scope also needs …". NOT a
  general impact-mapping or strategy-workshop tool — owns only this pipeline
  artifact.
---
...
**scope delta** (a 1–3 line change statement) arrives, do **not** run the Draft
pipeline (§c). Instead, **load `references/amend-mode.md` and follow it instead of
§c** — it is the complete Amend procedure (intent-first edit, minimal-diff
re-derivation, the delta-scoped professor gate, the harness `--baseline` run, and
the amendment report with the staleness note).
