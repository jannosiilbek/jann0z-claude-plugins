// md.mjs — hand-rolled, dependency-free markdown reader for the PINNED A-theme
// shapes the glossary harness must verify (simulation.md §1).
//
// COPIED (not cross-referenced) from the sibling event-storming skill's md.mjs:
// it must read the SAME pinned 01 upstream shape, and the same level-2/level-3
// heading + pipe-table + ordered-list primitives are exactly what the 02
// artifact (Terms / Enums / Forbidden Synonyms) needs. Each skill stays
// self-contained and copy-portable (doctrine: reuse over invention, copied
// never referenced).
//
...
// reports `malformed`; this reader itself only throws ParseError on a genuinely
// unreadable input (it is defensive, not the malformed authority).
...
// if it did, that is a content defect, not a parser concern).
...
export function parse(text) {
  ...
  // the SKILL.md contract; the `## Upstream Fingerprint` SECTION is the lint
  // authority, but expose the comment too).
  ...
  if (firstNonBlank !== -1 && lines[firstNonBlank].trim().startsWith('<!--')) {
    ...
    if (!closed) {
      throw new ParseError('fingerprint comment block opened (`<!--`) but never closed (`-->`)', 'L7');
    }
