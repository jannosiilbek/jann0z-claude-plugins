// md.mjs — hand-rolled, dependency-free markdown reader for the PINNED shapes
// the aggregates harness must verify (simulation.md §1).
//
// COPIED (not cross-referenced) from the sibling glossary/event-storming skills'
// md.mjs: this skill must read the SAME pinned 01 upstream shape, the SAME pinned
// 02 upstream shape, AND its own 03 S-theme shape — and the same level-2/level-3
// heading + pipe-table + ordered-list primitives cover all three. Each skill
// stays self-contained and copy-portable (doctrine: reuse over invention, copied
// never referenced).
//
