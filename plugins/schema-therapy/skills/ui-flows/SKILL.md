...
(32 rules, themes A–F, **22 ❌**); the executable oracle is
...
| each 08 model's `<Budget klm="…">` | a **ceiling** the realized flow cost must stay ≤ |
...
- KLM alphabet (shared with 08): `K P H B BB M W`; `nK` = multiplier (only before `K`). A full
  click **`BB` = 2 instances** (button-down + up); `count("MPBB") = M+P+B+B = 4`. Exactly **one
  leading `M`** per interaction/user Event, **zero** on system steps.
- **NAV_HOP_COST = `BB` = 2** per hop beyond the first container.
- **Conformance: `flow_cost ≤ model.Budget.klm`** (exact-value). Over-budget **fails simulation**.
  09 **never edits 08**; an over-budget flow is fixed by re-modeling 09 (or, if truly infeasible,
  an 08 regeneration — never a 09 budget edit).
...
4. **Simulation** — the **same harness** runs the flow-walk oracle: per realized 08 model it
   ...
   reachable (W-HOME), and **re-computes the flow cost vs the 08 budget** (W-COST). An
...
6. **Emit + report** — fixed-format report including a **per-model cost-vs-budget table** and
...
- **stdout**: one stable-key-order JSON summary (per-entity `authority` block, `flows[]` with
  computed-cost + budget + hops + realizable, sorted `checks[]`/`findings[]`).
- **stderr**: diagnostics only (parse throws, stranded-walk traces, over-budget detail with both
  values + the contributing path, upstream-defect routing note).
