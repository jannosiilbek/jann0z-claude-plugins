...
// used ONLY when `--upstream-05` is present, to read each `<entity>.scxml`'s transition
// set (From-state, `<!-- 01-event: … -->` annotation, To-state) as the lifecycle
// AUTHORITY for promoted entities (simulation.md §1, §3.5).
//
// This is the statecharts skill's reader PATTERN, COPIED — NEVER imported (isolation
// directive; each skill stays self-contained). It reads STRUCTURE/SHAPE only; the 06
// harness does NOT re-run SCXML on an engine (05's own correctness was certified by the
// statecharts harness). 06 consumes 05's transition set as the authority. The reader
// double-checks well-formedness: malformed XML throws ScxmlParseError ⇒ an unreadable
// supplied 05 routes to broken-test (§9).
...
function buildTree(tokens) {
  ...
  return { root, declSeen };
}

// parse(src) → a structural model. Throws ScxmlParseError on malformed XML.
// model = { states:[{id, kind}], finals:[id], transitions:[{sourceId, event, target, event01}],
//           basicStateIds:Set, allComments:[text] }
    ...
    if (STATE_KINDS.has(node.name)) {
      ...
      for (const c of node.children) {
        if (c.name === 'transition') {
          model.transitions.push(readTransition(c, id));
        }
      }
      for (const c of node.children) {
...
export function scxmlGraph(model) {
  ...
  const states = model.states.map((s) => s.id).filter(Boolean);
  const froms = new Set(transitions.map((t) => t.from));
  const terminals = [];
  for (const f of model.finals) terminals.push(f);
